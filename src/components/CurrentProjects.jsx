...
const CurrentProjects = ({ projects = [], employees = [] }) => {
  const [projectList, setProjectList] = useState(
    projects.filter(proj => proj.status === 'Đang thực hiện')
  );
  ...
  useEffect(() => {
    ...
    if (progressFilter !== 'all') {
      filtered = filtered.filter(proj => {
        if (progressFilter === '0-50') return proj.progress >= 0 && proj.progress <= 50;
        if (progressFilter === '51-100') return proj.progress >= 51 && proj.progress <= 100;
        return true;
      });
    }
    setFilteredProjects(filtered);
  }, [searchQuery, progressFilter, projectList]);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Ngày bắt đầu: {project.startDate}</p>
  ...
  return (
    <div className="space-y-8 p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Dự án hiện tại</h1>
        <button
